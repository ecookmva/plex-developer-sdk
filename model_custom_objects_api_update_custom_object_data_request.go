/*
Plex Developer API

Auto-generated OpenAPI spec from developers.plex.com

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package plex

import (
	"encoding/json"
)

// checks if the CustomObjectsApiUpdateCustomObjectDataRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CustomObjectsApiUpdateCustomObjectDataRequest{}

// CustomObjectsApiUpdateCustomObjectDataRequest struct for CustomObjectsApiUpdateCustomObjectDataRequest
type CustomObjectsApiUpdateCustomObjectDataRequest struct {
	// The custom fields and values for a custom object. The customFields property is an object with a schema derived from the relevant custom fields. Each property is the name of a custom field (not case sensitive) with a corresponding value matching the custom field's type. Custom Field Type to JSON Type Mapping  Boolean - boolean Custom Object Reference -  object Date - string(date) Date Time - string(date-time) Decimal - number(double) Integer - integer(int64) Standard Object Reference - object Text - string Time - string(time)  Custom Object Reference Format Custom Object References are always returned to the client as an object with three properties OR null. Properties  id - string(uuid) - The uuid used to uniquely identify a custom object record. recordIdentifier - integer(int64) - The value used to uniquely identify a custom object record. displayValue - string - This is either the object's Primary Field (when specified) or the object's first field.  When adding or updating a custom object reference, several formats can be used.  A string(uuid) representing the record's ID.  Example {   &quot;customFields&quot;: {     &quot;MyCustomObjectReference&quot;: &quot;d13023f2-9841-4439-8f8b-2d75ef3e87f4&quot;   } }   A integer(int64) representing a record identifier.  Example {   &quot;customFields&quot;: {     &quot;MyCustomObjectReference&quot;: 15   } }   An object with either a recordIdentifier or id property. Additional properties (such as displayName) are ignored with write operations.  Example {   &quot;customFields&quot;: {     &quot;MyCustomObjectReference&quot;: {       &quot;recordIdentifier&quot;: 15,       &quot;id&quot;: &quot;d13023f2-9841-4439-8f8b-2d75ef3e87f4&quot;,       &quot;displayValue&quot;: &quot;Blue&quot;     }   } }  Standard Object Reference Format Standard Object References are always returned to the client as an object with two properties OR null. Properties  recordIdentifiers - object - An object of identity keys and their values that uniquely identify a standard object record.  This object has a schema that is derived from the identifiers used by the referenced standard object. For example, the Part standard object has a single identifier named Part_Key. displayValue -  string - The human readable text used to represent the referenced record. This value alone may not uniquely identify a record.  When adding or updating a standard object reference, several formats can be used.  An object with the proper recordIdentifiers object. Additional properties (such as displayName) are ignored with write operations.  Example {   &quot;customFields&quot;: {     &quot;MyPartReference&quot;: {       &quot;recordIdentifiers&quot;: {         &quot;Part_Key&quot;: 1234       },       &quot;displayValue&quot;: &quot;4799-AAA&quot;     }   } }   When the referenced object only has one identifier, the single value may be used.  Example {   &quot;customFields&quot;: {     &quot;MyPartReference&quot;: 1234   } }
	CustomFields map[string]interface{} `json:"customFields,omitempty"`
}

// NewCustomObjectsApiUpdateCustomObjectDataRequest instantiates a new CustomObjectsApiUpdateCustomObjectDataRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCustomObjectsApiUpdateCustomObjectDataRequest() *CustomObjectsApiUpdateCustomObjectDataRequest {
	this := CustomObjectsApiUpdateCustomObjectDataRequest{}
	return &this
}

// NewCustomObjectsApiUpdateCustomObjectDataRequestWithDefaults instantiates a new CustomObjectsApiUpdateCustomObjectDataRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCustomObjectsApiUpdateCustomObjectDataRequestWithDefaults() *CustomObjectsApiUpdateCustomObjectDataRequest {
	this := CustomObjectsApiUpdateCustomObjectDataRequest{}
	return &this
}

// GetCustomFields returns the CustomFields field value if set, zero value otherwise.
func (o *CustomObjectsApiUpdateCustomObjectDataRequest) GetCustomFields() map[string]interface{} {
	if o == nil || IsNil(o.CustomFields) {
		var ret map[string]interface{}
		return ret
	}
	return o.CustomFields
}

// GetCustomFieldsOk returns a tuple with the CustomFields field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomObjectsApiUpdateCustomObjectDataRequest) GetCustomFieldsOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.CustomFields) {
		return map[string]interface{}{}, false
	}
	return o.CustomFields, true
}

// HasCustomFields returns a boolean if a field has been set.
func (o *CustomObjectsApiUpdateCustomObjectDataRequest) HasCustomFields() bool {
	if o != nil && !IsNil(o.CustomFields) {
		return true
	}

	return false
}

// SetCustomFields gets a reference to the given map[string]interface{} and assigns it to the CustomFields field.
func (o *CustomObjectsApiUpdateCustomObjectDataRequest) SetCustomFields(v map[string]interface{}) {
	o.CustomFields = v
}

func (o CustomObjectsApiUpdateCustomObjectDataRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CustomObjectsApiUpdateCustomObjectDataRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CustomFields) {
		toSerialize["customFields"] = o.CustomFields
	}
	return toSerialize, nil
}

type NullableCustomObjectsApiUpdateCustomObjectDataRequest struct {
	value *CustomObjectsApiUpdateCustomObjectDataRequest
	isSet bool
}

func (v NullableCustomObjectsApiUpdateCustomObjectDataRequest) Get() *CustomObjectsApiUpdateCustomObjectDataRequest {
	return v.value
}

func (v *NullableCustomObjectsApiUpdateCustomObjectDataRequest) Set(val *CustomObjectsApiUpdateCustomObjectDataRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableCustomObjectsApiUpdateCustomObjectDataRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableCustomObjectsApiUpdateCustomObjectDataRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCustomObjectsApiUpdateCustomObjectDataRequest(val *CustomObjectsApiUpdateCustomObjectDataRequest) *NullableCustomObjectsApiUpdateCustomObjectDataRequest {
	return &NullableCustomObjectsApiUpdateCustomObjectDataRequest{value: val, isSet: true}
}

func (v NullableCustomObjectsApiUpdateCustomObjectDataRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCustomObjectsApiUpdateCustomObjectDataRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


